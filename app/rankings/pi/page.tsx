...
export default function PiRankingsPage() {
  ...
  const sorted = [...teams].sort((a, b) => b.piOverall - a.piOverall);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">Pi-Ratings</h1>
        <p className="text-sm text-gray-400">
          Match-history ratings with separate home/away strength (Constantinou & Fenton, 2013)
        </p>
            ...
            <table className="tr-table">
              <thead>
                <tr>
                  ...
                  <th className="text-right w-[70px] hidden md:table-cell">Combined</th>
                  <th className="text-right w-[50px] hidden lg:table-cell">Comb #</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((team, i) => {
                  const split = team.piHome - team.piAway;
                  return (
                    <tr
                      key={team.id}
                    ...
                    >
                      ...
                      <td className="text-right">
                        <span className="font-mono font-semibold tabular-nums">
                          {team.piOverall.toFixed(2)}
                        </span>
                      </td>
                      <td className="text-right">
                        <span className="font-mono tabular-nums text-gray-500">
                          {team.piHome.toFixed(2)}
                        </span>
                      </td>
                      <td className="text-right">
                        <span className="font-mono tabular-nums text-gray-500">
                          {team.piAway.toFixed(2)}
                        </span>
                      </td>
