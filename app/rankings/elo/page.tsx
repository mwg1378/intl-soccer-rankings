...
export default function EloRankingsPage() {
  ...
  // Compute Elo-only overall and sort
  const sorted = teams
    .map((t: Team) => ({
      ...t,
      eloOverall: (t.eloOffensive + (3000 - t.eloDefensive)) / 2,
    }))
    .sort((a, b) => b.eloOverall - a.eloOverall);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">Elo-Only Rankings</h1>
        ...
        <>
          <div className="overflow-x-auto rounded border border-gray-200">
            <table className="tr-table">
              <thead>
                <tr>
                  <th className="w-[50px]">Elo Rank</th>
                  <th>Team</th>
                  <th className="w-[70px]">Conf</th>
                  <th className="text-right w-[80px]">Elo Rating</th>
                  <th className="text-right w-[80px] hidden md:table-cell">Combined</th>
                  <th className="text-right w-[60px] hidden md:table-cell">Comb. Rank</th>
                  <th className="text-right w-[60px] hidden lg:table-cell">Home Adv.</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((team, i) => (
                  ...
                  >
                    ...
                    <td className="text-right hidden lg:table-cell">
                      <span className="font-mono tabular-nums text-gray-400 text-xs">
                        {team.homeAdvantage.toFixed(2)}x
                      </span>
                    </td>
                  </tr>
                ))}
