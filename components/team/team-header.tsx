  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-3xl font-extrabold tabular-nums text-[#399F49]">
            #{team.currentRank}
          </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {ratingCards.map((card) => (
          <div key={card.label} className="overflow-hidden rounded border border-gray-200">
            <div className="bg-[#399F49] px-3 py-1.5">
              <span className="text-[11px] font-semibold uppercase tracking-wide text-white">
                {card.label}
              </span>
            </div>
            <div className="px-3 py-2">
              <span className={`text-2xl font-bold tabular-nums ${card.highlight ? "text-[#399F49]" : "text-gray-600"}`}>
                {card.value}
              </span>
            </div>
          </div>
